...
Solid Table hasn't been published to NPM registry yet, but it will soon be available as `solid-table`, and you'll be able to add it like:

```plaintext
npm install solid-table
```

## Quick Start

At the heart of every Solid Table is the `createTable` function and the table instance object that it returns. This instance object contains everything you'll need to build a table and interact with its state.
